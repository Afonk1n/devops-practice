...
## Блок 0: Рабочее окружение (WSL + Ubuntu)

**Что мы использовали:**
- Windows 10/11 с WSL2 (Windows Subsystem for Linux)
- Ubuntu в WSL2 для выполнения команд
- Docker Desktop с интеграцией WSL2

**Почему WSL2:**
- Kubernetes и Docker лучше работают в Linux-окружении
- Большинство команд DevOps выполняются в Linux-терминале
- WSL2 даёт полноценный Linux без виртуальной машины

**Базовые команды для работы:**
- `wsl` — запустить Ubuntu из Windows CMD
- `cd /mnt/d/DevOpsPractice` — перейти в папку проекта (диск D: монтируется как `/mnt/d/`)
- `sudo docker` — выполнить команды Docker (нужны права администратора)

**Важно для DevOps:**
- Ты должен уметь работать в Linux-терминале (основные команды: `cd`, `ls`, `pwd`, `cat`, `grep`)
- Понимать разницу между Windows и Linux (пути, права доступа)
- Знать, как использовать WSL для работы с Docker и Kubernetes

**Потенциальные вопросы на собесе:**
1. **Почему используешь WSL, а не виртуальную машину?** — WSL2 быстрее, интегрирован с Windows, не требует отдельной ОС.
2. **Как работаешь с файлами между Windows и WSL?** — Файлы на дисках Windows доступны через `/mnt/c/`, `/mnt/d/` и т.д.
3. **Почему нужен `sudo` для Docker?** — Docker требует права администратора для доступа к Docker daemon.
...
**Что защищает APP_KEY:**
- Сессии пользователей (cookies зашифрованы этим ключом)
- Данные форм (CSRF-токены)
- Зашифрованные данные в базе (если используешь `encrypt()` в Laravel)
- Без этого ключа Laravel не может работать безопасно — все сессии и зашифрованные данные будут недоступны.
...
**Что такое CI/CD:**
- **CI (Continuous Integration)** — непрерывная интеграция: автоматическая проверка кода при каждом коммите (сборка, тесты, линтеры). Мы настроили CI: при push в master запускаются тесты и сборка Docker-образа.
- **CD (Continuous Deployment/Delivery)** — непрерывное развёртывание: автоматический деплой в продакшен после успешного CI. Мы не настраивали CD, потому что для этого нужен доступ к продакшен-кластеру и настройка деплоя через Helm/ArgoCD. В реальном проекте CD автоматически разворачивает приложение после успешных тестов.
...
- Redis — это **база данных в памяти** (in-memory database), очень быстрая, потому что данные хранятся в RAM, а не на диске.
- В Laravel используется для:
  - **Кеширования** — сохранение результатов запросов к базе, чтобы не делать их снова (быстрее)
  - **Очередей** — фоновые задачи (отправка email, обработка изображений) выполняются асинхронно
  - **Сессий** — хранение данных сессий пользователей (альтернатива файлам или базе)
- Для DevOps достаточно понимать: Redis — это быстрый кеш/очередь, работает в памяти, данные могут пропасть при перезапуске (но это нормально для кеша).
...
**Что значит "проксирует" (простыми словами):**
- Проксирование — это когда один сервер (Nginx) **принимает запрос от клиента** и **передаёт его дальше** другому серверу (PHP-FPM), получает ответ и возвращает клиенту.
- Простыми словами: Nginx — это "посредник" между браузером и PHP-FPM. Браузер не общается напрямую с PHP-FPM, а только с Nginx, который уже решает, куда направить запрос.
...
Docker-compose позволяет описать всю инфраструктуру в одном файле и запустить её одной командой. Это основа для понимания, как сервисы взаимодействуют друг с другом.
...
**Volumes для разработки vs продакшена:**
- **В разработке:** используем `volumes` для монтирования кода с компьютера в контейнер — изменения видны сразу, не нужно пересобирать образ.
- **В продакшене:** код **зашит в образ** при сборке (`COPY src /var/www/html` в Dockerfile) — это гарантирует, что на всех серверах один и тот же код, образ неизменяемый (immutable). Volumes в проде используются только для данных (база, логи), а не для кода.
...
**Зачем именно такая композиция сервисов:**
...
**Что такое тесты (кратко):**
- Тесты — это автоматические проверки, что код работает правильно (например, "при создании пользователя он должен появиться в базе"). 
- Для DevOps важно понимать: тесты запускаются в CI автоматически, если тесты не прошли — деплой не должен происходить. Ты не пишешь тесты (это зона разработчиков), но должен понимать, что они есть и зачем нужны.
...
**Что такое `uses` в GitHub Actions:**
- `uses` — это указание на **готовое действие** (action) из GitHub Marketplace или репозитория. Это переиспользуемые компоненты, которые делают стандартные вещи.
- Примеры:
  - `actions/checkout@v4` — стандартное действие для клонирования репозитория
  - `docker/setup-buildx-action@v3` — настройка Docker Buildx для сборки образов
  - `shivammathur/setup-php@v2` — установка PHP с нужными расширениями
- Это стандартные действия, которые используют все, не нужно писать их с нуля.
...
- Ты умеешь описать pipeline, понимаешь, какие шаги важны (зависимости, миграции, тесты, сборка образа).

**Потенциальные вопросы на собесе:**
1. **Что такое CI/CD?** — CI (Continuous Integration) — автоматическая проверка кода при коммите (тесты, сборка), CD (Continuous Deployment) — автоматический деплой в продакшен после успешного CI.
2. **Почему используешь Docker для Laravel?** — Docker упаковывает приложение со всеми зависимостями (PHP, расширения, настройки) в образ, который можно запустить на любой машине без установки зависимостей вручную.
3. **Зачем нужен docker-compose?** — Описывает несколько контейнеров (app, nginx, db, redis) в одном файле и запускает их все вместе одной командой, настраивая сеть между ними автоматически.
...
**Типы Service (простыми словами):**
- **ClusterIP** (по умолчанию) — доступен только внутри кластера, другие поды могут обращаться по имени (например, `laravel-web:80`). Используется для внутренних сервисов.
- **NodePort** — открывает порт на каждом узле кластера, можно обращаться извне через `IP_узла:порт`. Используется для доступа извне, но не очень удобно (нужно знать IP узла).
- **LoadBalancer** — создаёт внешний балансировщик нагрузки (в облаке автоматически получает внешний IP). Используется в продакшене для публичных сервисов.
...
**Потенциальные вопросы на собесе:**
1. **Что такое Pod, Deployment, Service?** — Pod — контейнер(ы) в Kubernetes, Deployment — управляет Pod'ами (количество, обновления), Service — постоянный адрес для доступа к Pod'ам.
2. **В чём разница между ConfigMap и Secret?** — ConfigMap — несекретные данные (настройки), Secret — секретные данные (пароли, ключи) в base64.
3. **Как обновить приложение в Kubernetes?** — Изменить образ в Deployment (`kubectl set image deployment/laravel-web app=laravel-app-app:v2`), Kubernetes постепенно обновит Pod'ы без простоя (rolling update).
...
**Манифесты vs Helm-чарты (реальная практика):**

**Короткий ответ:** Да, в реальных проектах манифесты из `k8s/` обычно **удаляют** после создания Helm-чарта, потому что Helm-чарт их полностью заменяет.

**Подробнее:**

1. **В продакшене:**
   - Хранят только Helm-чарт (`helm/laravel-app/`)
   - Манифесты из `k8s/` удаляют, потому что они дублируют то, что делает Helm
   - Helm-чарт генерирует те же манифесты, но с переменными, поэтому исходные манифесты не нужны

2. **Почему удаляют:**
   - **Избежание дублирования:** одни и те же ресурсы описаны в двух местах (манифесты и шаблоны)
   - **Источник истины:** только Helm-чарт — единственный источник конфигурации
   - **Упрощение:** не нужно синхронизировать изменения между манифестами и шаблонами

3. **Когда могут оставить:**
   - **Для документации:** чтобы показать, как выглядели манифесты до шаблонизации
   - **Для отладки:** чтобы сравнить, что генерирует Helm (`helm template`) с исходными манифестами
   - **Для Git history:** чтобы видеть историю изменений (но обычно это не нужно)

4. **В нашем проекте:**
   - Мы оставили манифесты в `k8s/` для наглядности (учебный проект)
   - В реальном проекте их бы удалили после создания Helm-чарта
   - Всё управление идёт через Helm (`helm install`, `helm upgrade`)

**Итог:** Технически манифесты не нужны, если есть Helm-чарт. Helm генерирует манифесты из шаблонов при установке/обновлении, поэтому исходные манифесты становятся избыточными.
...
**Потенциальные вопросы на собесе:**
1. **Почему Helm, а не просто kubectl apply?** — Helm даёт версионирование, параметризацию, возможность откатиться назад. В проде почти все используют Helm для сложных приложений.
2. **Что такое values.yaml?** — Файл с настройками по умолчанию, можно переопределить при установке через `--set` или отдельный файл.
3. **Нужно ли хранить манифесты после создания Helm-чарта?** — Нет, в реальных проектах манифесты удаляют, потому что Helm-чарт их полностью заменяет. Helm генерирует манифесты из шаблонов при установке, поэтому исходные манифесты становятся избыточными.
...
**Откуда берётся пароль:**
- При установке kube-prometheus-stack Helm-чарт автоматически создаёт Kubernetes Secret `prometheus-grafana` с паролем для пользователя `admin`.
- Пароль хранится в Secret в закодированном виде (base64), поэтому нужно декодировать через `base64 -d`.
- Это стандартное поведение: Helm-чарт генерирует случайный пароль и сохраняет его в Secret для безопасности.
...
**Потенциальные вопросы на собесе:**
1. **Что такое Prometheus и Grafana?** — Prometheus собирает метрики (CPU, память, сеть), Grafana визуализирует их в графиках и дашбордах.
2. **Как получить доступ к Grafana?** — Через `kubectl port-forward` для локального доступа или через Ingress для продакшена.
3. **Какие метрики важны для веб-приложений?** — CPU Usage, Memory Usage, Network Traffic. Высокий CPU или память = проблема, нужно масштабировать.
...
**Потенциальные вопросы на собесе:**
1. **В чём разница между liveness и readiness probe?** — Liveness проверяет, что приложение живое (если не отвечает — перезапускает под), readiness проверяет, что готово принимать трафик (если не готово — убирает из Service).
2. **Зачем нужны реплики?** — Для отказоустойчивости (если один под упадёт, второй продолжит работать) и распределения нагрузки.
3. **Что такое NetworkPolicy?** — Правило, которое ограничивает сетевой трафик между подами. Без него все поды могут общаться друг с другом, с NetworkPolicy — только разрешённые соединения.
